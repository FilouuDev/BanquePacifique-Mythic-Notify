...
local PoliceDoors = {
    ...
    {loc = vector3(252.72, 220.95, 101.68), txtloc = vector3(252.72, 220.95, 101.68), model = "hei_v_ilev_bk_safegate_pris", model2 = "hei_v_ilev_bk_safegate_molten", obj = nil, obj2 = nil, locked = false},
    {loc = vector3(261.01, 215.01, 101.68), txtloc = vector3(261.01, 215.01, 101.68), model = "hei_v_ilev_bk_safegate_pris", model2 = "hei_v_ilev_bk_safegate_molten", obj = nil, obj2 = nil, locked = false},
    ...
    if not info.locked then
        if (os.time() - cooldown) > lastrobbed then
            ...
            if copcount >= mincops then
                if method == 1 then
                    local item = yPlayer.getInventoryItem("c4_bank")["count"]

                    if item >= 1 then
                        yPlayer.removeInventoryItem("c4_bank", 1)
                        cb(true)
                        info.stage = 1
                        info.style = 1
                        info.locked = true
                ...
                cb("Il N'y a pas assez "..mincops.." de police pour faire cela.")
            end
        else
